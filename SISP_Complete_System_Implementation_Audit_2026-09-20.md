...
3. **CRITICAL source-level privacy issue — public admission status serializes a User record including a password hash, and activation code has a weak identity-verification path.** Deployed HEAD contains these code paths, but the live database is missing the three admission tables expected by its Prisma schema, so successful production endpoint behavior is UNVERIFIED; no actual disclosure or takeover was attempted or demonstrated. The worktree contains safer candidate implementations.
...
Local Prisma schema has 36 models, including role/permission tables; users/audit; student/program/curriculum/course/prerequisites; academic term, semester, enrollment/history, grade, balances; requests/catalog/items/payment transaction; notifications; ARIA chat/escalation/session/quota; admissions; class sections/schedules; adviser assignment/concerns; knowledge documents/chunks. Deployed HEAD’s Prisma schema has 29 models; read-only live public inventory has 27 tables, including extra `events`, but lacks the three admission tables expected by HEAD. The dirty local schema expects ten missing tables total (those three plus seven new supporting tables). Compared with HEAD schema, live columns are also missing `student_profiles.curriculum_id`, three payment-proof columns on `document_requests`, and `fee_note`, `tat`, `assigned_to` on `document_catalog_items`. The candidate schema expects 11 additional columns not yet live: four audit snapshot/value columns, six enrollment-history fields, and `enrollments.class_section_id`. Live Supabase checks show 6 role rows, 0 permission rows, 0 role-permission links, 86 active users. Six named demo accounts previously requested are absent. Production role counts: `student` 59, `faculty` 10, `dean` 4, `sys_admin` 3, `admin_staff` 5, `live_agent` 5. There are no separate live `registrar` or `treasury` roles.
...
| ADMIT-01 | Admission application/status/document evidence | Applicant/Registrar | `/admission`; admission APIs; application/submission models | HEAD expects three admission tables absent from live `public`; no production flow | PARTIAL | HIGH — source-level serializer/identity defects; runtime currently unverified and may fail on missing tables |
...
- **CRITICAL code defect:** deployed public admission-status serializer includes a `User` row containing `passwordHash`, per the HEAD service path. The live DB currently lacks the three admission tables expected by HEAD; successful disclosure was not demonstrated, and endpoint behavior is UNVERIFIED. Current dirty candidate selects safe applicant fields.
- **CRITICAL code defect:** deployed activation path has weaker identity verification and returns a temporary password when its required DB records are available. The same missing admission-table schema prevents claiming the live route succeeds; no production activation was attempted. Candidate source requires the recorded verified admission record and never returns a plaintext activation password.
...
This addendum records checks run after the main audit report was drafted. It does not imply any production remediation. This checkout is `defense-candidate` at `655f08a1`; the audit's candidate-vs-live counts below refer to the recovery-plan snapshot and are not a fresh check of this branch. In particular, the earlier note about a correction on `codex/sisp-foundation-recovery` is historical evidence, not proof that the correction exists in or is deployed from the current checkout.

### Migration and extension evidence

- Supabase migration history currently returns eight entries. Local Prisma contains 15 migration folders, several with timestamps different from the listed live versions. `public._prisma_migrations` is absent. Do not assume matching names mean matching effects or that `prisma migrate deploy` is ready. In this checkout, deployed `origin/main` defines 29 models and candidate `HEAD` defines 36; compare each migration and field against an authoritative live catalog before applying anything.
...
## Migration/plan execution addendum — 2026-09-21

A further read-only pass compared the 15 working-tree Prisma migration SQL files with the plan and the prior live catalog evidence. It confirmed several data-writing migrations. The SQL files are tracked in this candidate repository, but `migration_lock.toml` is absent; add/review lock metadata as part of migration-chain reconciliation. `db push` would reconcile modeled structure only; it would not replay seed/backfill SQL or register migration history. The generated schema diff also proposes dropping `events` (currently empty per prior read-only count), creating ten absent tables, and changing selected types, nullability, defaults, constraints, and indexes. No columns are dropped, and the diff contains no row DML. This does not make the push safe: policies, role/permission rows, school calendar/program records, and migration history would remain divergent.

The reviewed data writes include document catalog fees, derived academic terms and term backfills, a hard-coded 2026–2027 active term, seven named programs, and RBAC role/permission seeding. The `courses_code_key` constraint is dropped and replaced by `(code,title)` uniqueness. These effects need institution/source confirmation and exact live-state mapping. The role migration still conflicts with 29 live policy definitions mentioning `admin_staff`; financial policies require an explicit Treasury split under DEC-012.

No local PostgreSQL, Docker/Podman, psql, initdb, or Supabase CLI was available. No staging resource was created because the only Supabase branch option has a recurring cost and the user requested no cost. This pass changed no application or migration SQL and performed no production writes. Production `db push`/`migrate deploy` remains BLOCKED pending migration ledger/schema reconciliation, policy and seed reconciliation, an isolated free PostgreSQL+pgvector rehearsal, and backup/restore verification.

### Candidate production/mock isolation fix — 2026-09-21

A narrow code correction in `sisp-backend/src/prisma/prisma.service.ts` prevents production `PrismaService` construction from generating mock account hashes, initializing demo fixtures, reading `mock-db.json`, or setting up its periodic JSON persistence. The existing strict production database-connect behavior remains fail closed. This change is local and uncommitted, not deployed. `git diff --check` passed. Backend build was attempted but `prisma generate` failed before TypeScript compilation with Windows `EPERM` renaming a locked `query_engine-windows.dll.node`. Backend `npx tsc --noEmit` emitted no errors but its shell session ended without an exit code, so that check is inconclusive. No test was run. Existing ignored mock JSON was neither read nor changed by this review.
